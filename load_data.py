...
def _load_GQA(dataroot, name, img_id2val, label2ans, adaptive=True):
    """Load entries
    ...
    if os.path.isfile(cache_path):
        ...
        print('Loading GQA %s' % name)
        print('\tUsed GQA images: %d/%d (%.4f)' %
              (counts[0], len(_vgv), counts[0] / len(_vgv)))
        print('\tOut-of-split GQA images: %d/%d (%.4f)' %
              (counts[3], counts[0], counts[3] / counts[0]))
        print('\tUsed GQA questions: %d/%d (%.4f)' %
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, relation_type, dataroot='data',
        ...
        with h5py.File(h5_path, 'r') as hf:
            ...
            if "action_adj_matrix" in hf.keys() \
                    and self.relation_type == "action":
                self.action_adj_matrix = np.array(
                    hf.get('action_adj_matrix'))
                print("Loaded action adj matrix from file...",
                      self.action_adj_matrix.shape)
            else:
                self.action_adj_matrix = None
                print("Setting action adj matrix to None...")
            ...
            if "attribute_adj_matrix" in hf.keys() \
                    and self.relation_type == "action":
                self.attribute_adj_matrix = np.array(
                    hf.get('attribute_adj_matrix'))
                print("Loaded action adj matrix from file...",
                      self.attribute_adj_matrix.shape)
            else:
                self.attribute_adj_matrix = None
                print("Setting attribute adj matrix to None...")
...
class GQADataset(Dataset):
    def __init__(self, name, features, normalized_bb, bb,
        ...
        self.entries = _load_GQA(dataroot, name, self.img_id2idx,
                                 self.label2ans,
...
def tfidf_from_questions(names, dictionary, dataroot='data',
    ...
    # GQA
    if 'vg' in target:
        question_path = os.path.join(dataroot, 'GQA',
...
def _load_VQACE(dataroot, name, img_id2val, label2ans, adaptive=True):
    """Load entries
    ...
    """
    question_path = os.path.join(dataroot,
                                 'VQA-CE/question_answers.json')
    image_data_path = os.path.join(dataroot,
                                   'VQA-CE/image_data.json')
    ans2label_path = os.path.join(dataroot, 'cache', 'trainval_ans2label.pkl')
    cache_path = os.path.join(dataroot, 'cache', 'vg_%s%s_target.pkl' %
                              (name, '_adaptive' if adaptive else ''))
    ...
    if os.path.isfile(cache_path):
        ...
        print('Loading VQA-CE %s' % name)
        print('\tUsed VQA-CE images: %d/%d (%.4f)' %
              (counts[0], len(_vgv), counts[0] / len(_vgv)))
        print('\tOut-of-split VQA-CE images: %d/%d (%.4f)' %
              (counts[3], counts[0], counts[3] / counts[0]))
        print('\tUsed VQA-CE questions: %d/%d (%.4f)' %
              (counts[1], counts[2], counts[1] / counts[2]))
        with open(cache_path, 'wb') as f:
            pickle.dump(entries, open(cache_path, 'wb'))
...
class VQACEDataset(Dataset):
    def __init__(self, name, features, normalized_bb, bb,
                 ...
                 relation_type, dataroot='data', adaptive=False,
                 pos_boxes=None, pos_emb_dim=64):
        super(VQACEDataset, self).__init__()
        # do not use test split images!
        assert name in ['train', 'val']
        print('loading VQA-CE data %s' % name)
        ans2label_path = os.path.join(dataroot, 'cache',
                                      'trainval_ans2label.pkl')
        label2ans_path = os.path.join(dataroot, 'cache',
                                      'trainval_label2ans.pkl')
        ...
        self.entries = _load_VQACE(dataroot, name, self.img_id2idx,
                                      self.label2ans,
                                      adaptive=self.adaptive)
        self.tokenize()
        self.tensorize()
...
def _load_VCR(dataroot, name, img_id2val, label2ans, adaptive=True):
    """Load entries
    ...
    if os.path.isfile(cache_path):
        ...
        print('Loading VCR %s' % name)
        print('\tUsed VCR images: %d/%d (%.4f)' %
              (counts[0], len(_vgv), counts[0] / len(_vgv)))
        print('\tOut-of-split VCR images: %d/%d (%.4f)' %
              (counts[3], counts[0], counts[3] / counts[0]))
        print('\tUsed VCR questions: %d/%d (%.4f)' %
              (counts[1], counts[2], counts[1] / counts[2]))
        with open(cache_path, 'wb') as f:
            pickle.dump(entries, open(cache_path, 'wb'))
...
class VCRDataset(Dataset):
    def __init__(self, name, features, normalized_bb, bb,
        ...
        self.entries = _load_VCR(dataroot, name, self.img_id2idx,
                                 self.label2ans,
                                 adaptive=self.adaptive)
        self.tokenize()
        self.tensorize()
...
def _load_VQAX(dataroot, name, img_id2val, label2ans, adaptive=True):
    """Load entries
    ...
    """
    question_path = os.path.join(dataroot,
                                 'VQA-X/question_answers.json')
    image_data_path = os.path.join(dataroot,
                                   'VQA-X/image_data.json')
    ans2label_path = os.path.join(dataroot, 'cache', 'trainval_ans2label.pkl')
    cache_path = os.path.join(dataroot, 'cache', 'vg_%s%s_target.pkl' %
                              (name, '_adaptive' if adaptive else ''))
    ...
    if os.path.isfile(cache_path):
        ...
        print('Loading VQA-X %s' % name)
        print('\tUsed VQA-X images: %d/%d (%.4f)' %
              (counts[0], len(_vgv), counts[0] / len(_vgv)))
        print('\tOut-of-split VQA-X images: %d/%d (%.4f)' %
              (counts[3], counts[0], counts[3] / counts[0]))
        print('\tUsed VQA-X questions: %d/%d (%.4f)' %
              (counts[1], counts[2], counts[1] / counts[2]))
        with open(cache_path, 'wb') as f:
            pickle.dump(entries, open(cache_path, 'wb'))
...
class VQAXDataset(Dataset):
    def __init__(self, name, features, normalized_bb, bb,
        ...
        # do not use test split images!
        assert name in ['train', 'val']
        print('loading VQA-X data %s' % name)
        ans2label_path = os.path.join(dataroot, 'cache',
                                      'trainval_ans2label.pkl')
        label2ans_path = os.path.join(dataroot, 'cache',
                                      'trainval_label2ans.pkl')
        ...
        self.entries = _load_VQAX(dataroot, name, self.img_id2idx,
                                     self.label2ans,
                                     adaptive=self.adaptive)
        self.tokenize()
        self.tensorize()
...
class CLEVRXDataset(Dataset):
    def __init__(self, name, features, normalized_bb, bb,
                 ...
                 relation_type, dataroot='data', adaptive=False,
                 pos_boxes=None, pos_emb_dim=64):
        super(CLEVRXDataset, self).__init__()
        # do not use test split images!
        assert name in ['train', 'val']
        ...
        self.entries = _load_CLEVRX(dataroot, name, self.img_id2idx,
                                       self.label2ans,
                                       adaptive=self.adaptive)
        self.tokenize()
        self.tensorize()
